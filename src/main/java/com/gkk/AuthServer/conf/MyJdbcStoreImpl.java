...
/**
 * 此处可覆盖spring secrity默认实现的jdbc方法，实现自定义token存储
 * @author lenovo
 *
 */
public class MyJdbcStoreImpl implements TokenStore {

	private static final Log LOG = LogFactory.getLog(MyJdbcStoreImpl.class);
	
	
	//自定义token存储规则
	private static final String myTokenStoreSql="INSERT INTO ACCESS_TOKEN(ID,TOKEN,USER_NAME,CLIENT_ID) VALUES(?,?,?,?)";
	...
	public MyJdbcStoreImpl(DataSource dataSource) {
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		LOG.info("-------------------------------进入自定义storeAccessToken方法------------------------------------------");
		...
		//token的过期时间、类型等需要的字段可自行添加扩展
		jdbcTemplate.update(myTokenStoreSql, new Object[] {UUID.randomUUID().toString(),
				token.getValue(), authentication.isClientOnly() ? null : authentication.getName(),authentication.getOAuth2Request().getClientId()
				 }, new int[] {
